...
const ComplaintList = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const res = await axios.get("https://quirkyroomie-backend-1.onrender.com/api/complaints", {
  ...
  const voteComplaint = async (id, voteType) => {
    ...
    try {
      await axios.post(
        `https://quirkyroomie-backend-1.onrender.com/api/complaints/${id}/vote`,
        { vote: voteType },
        { headers: { Authorization: localStorage.getItem("token") } }
      );
  ...
  const resolveComplaint = async (id) => {
    ...
    try {
      await axios.put(
        `https://quirkyroomie-backend-1.onrender.com/api/complaints/${id}/resolve`,
        {},
        { headers: { Authorization: localStorage.getItem("token") } }
      );
